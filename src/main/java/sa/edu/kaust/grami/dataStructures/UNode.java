...
public class UNode implements Node<Integer, Double> {
    ...
    private HashMap<Integer, ArrayList<Pair<Integer, Double>>> reachableNodes; // Label~<nodeID,edge_label, edge_weight>, outgoing nodes
    private HashMap<Integer, ArrayList<Pair<Integer, Double>>> reachedBYNodes; // Label~<nodeID,edge_label, edge_weight>, ingoing nodes
    ...
    public void addreachableNode(UNode node, double edgeLabel) {
        if (reachableNodes == null) {
            reachableNodes = new HashMap<Integer, ArrayList<Pair<Integer, Double>>>();
        }

        ArrayList<Pair<Integer, Double>> list = reachableNodes.get(node.getLabel());
        if (list == null) {
            list = new ArrayList<Pair<Integer, Double>>();
            reachableNodes.put(node.getLabel(), list);
        }
        if (!list.contains(node.getID())) {
            list.add(new Pair<Integer, Double>(node.getID(), edgeLabel));
        }
    ...
    private void addreachedBYNodes(UNode node, double edgeLabel) {
        if (reachedBYNodes == null) {
            reachedBYNodes = new HashMap<Integer, ArrayList<Pair<Integer, Double>>>();
        }
        ArrayList<Pair<Integer, Double>> list = reachedBYNodes.get(node.getLabel());
        if (list == null) {
            list = new ArrayList<Pair<Integer, Double>>();
    ...
    public void printOutReachableNodes() {
        ...
        for (ArrayList<Pair<Integer, Double>> arr : reachableNodes.values()) {
    ...
    public ArrayList<Pair<Integer, Double>> getRechableWithEdgeLabel(double edgeLabel) {
        if (reachableNodes == null) {
            return new ArrayList<Pair<Integer, Double>>();
        }

        ArrayList<Pair<Integer, Double>> tempArr = new ArrayList<Pair<Integer, Double>>();
        for (Map.Entry<Integer, ArrayList<Pair<Integer, Double>>> entry : reachableNodes.entrySet()) {
            tempArr.addAll(entry.getValue());
        }
        for (int j = 0; j < tempArr.size(); j++) {
            Pair<Integer, Double> mp = tempArr.get(j);
    ...
    public ArrayList<Pair<Integer, Double>> getRechableWithNodeIDs(int label, double edgeLabel) {
        if (reachableNodes == null) {
            return new ArrayList<Pair<Integer, Double>>();
        }
    ...
    public HashMap<Integer, ArrayList<Pair<Integer, Double>>> getReachableWithNodes() {
        return reachableNodes;
    }

    public HashMap<Integer, ArrayList<Pair<Integer, Double>>> getReachableByNodes() {
        return reachedBYNodes;
    }

    public ArrayList<Pair<Integer, Double>> getRechableByNodeIDs(int label, double edgeLabel) {
        if (reachedBYNodes == null) {
            return new ArrayList<Pair<Integer, Double>>();
        }
        ArrayList<Pair<Integer, Double>> tempArr = new ArrayList<Pair<Integer, Double>>();

...
public class HPListGraph<NodeType, EdgeType> implements
    ...
    private double[][] max_edge_weights_map;
    ...
    private final int _addEdge(final int na, final int nb,
        ...
        max_edge_weights_map[idx] = maxWeights;
        edgesChanged();
        return idx;
    }
    ...
    public double[] getMaxEdgeWeights(int na, int nb) {
        int edgeIdx = getEdge(na, nb);
        if (edgeIdx < 0) {
            return null;
        } else {
            return getMaxEdgeWeights(edgeIdx);
        }
    }

    public double getMaxEdgeWeight(int na, int nb, int index) {
        int edgeIdx = getEdge(na, nb);
        if (edgeIdx < 0) {
            return -1;
        } else {
            return getMaxEdgeWeight(edgeIdx, index);
        }
    }
    ...
    public void setFreqStatus(Map<NodeType, ArrayList<Integer>> embeddings) {
        ...
        for (ArrayList<Integer> embs : embsList) {
    ...
    public int addEdgeIndex(final int nodeAIdx, final int nodeBIdx,
            final EdgeType label, final double[] maxWeights, final int direction) {
        return _addEdge(nodeAIdx, nodeBIdx, label, maxWeights, direction);
    }
    ...
    public int addNodeAndEdgeIndex(final int nodeAIdx,
            ...
            final double[] maxWeights, final int direction) {

        final int nodeBIdx = addNodeIndex(nodeLabel);
        _addEdge(nodeAIdx, nodeBIdx, edgeLabel, maxWeights, direction);
        return nodeBIdx;
    }
    ...
    public double getMaxEdgeWeight(final int edgeIdx, final int idx) {
        return max_edge_weights_map[edgeIdx][idx];
    }

    public double[] getMaxEdgeWeights(final int edgeIdx) {
        return max_edge_weights_map[edgeIdx];
    }

    public void setMaxEdgeWeights(final int edgeIdx, double[] weights) {
        max_edge_weights_map[edgeIdx] = weights;
        edgesChanged();
    }
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.parsemis.graph.HPGraph#setEdgeWeight(int, double)
     */
    public void setMaxEdgeWeight(final int edgeIdx, final double weight, final int idx) {
        max_edge_weights_map[edgeIdx][idx] = weight;
        edgesChanged();
    }

    public void setMinEdgeWeight(final int edgeIdx, final double weight, final int idx) {
//        min_edge_weights_map[edgeIdx][idx] = weight;
//        edgesChanged();
    }

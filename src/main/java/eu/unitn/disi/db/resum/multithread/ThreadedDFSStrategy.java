...
import eu.unitn.disi.db.resum.search.Strategy;
...
public class ThreadedDFSStrategy<NodeType, EdgeType> extends
        ...
        StackList<NodeType, EdgeType>, Generic<NodeType, EdgeType>,
        Strategy<NodeType, EdgeType> {

    private final Collection<HPListGraph<NodeType, EdgeType>>[] answers;
    
    private final ArrayList<BitSet>[] bulkRets;
     ...
     */
    public ThreadedDFSStrategy(final int threadcount) {
        next = prev = this;
        this.answers = new ArrayList[threadcount];
        this.bulkRets = new ArrayList[threadcount];
    ...
    */
    public Collection<HPListGraph<NodeType, EdgeType>> search(final Algorithm<NodeType, EdgeType> algo) {
        final MiningStack<NodeType, EdgeType> mine = new LocalStack<NodeType, EdgeType>(0, this, LocalEnvironment.env(this));
        ...
        answers[0] = new ArrayList<HPListGraph<NodeType, EdgeType>>();
        final Worker<NodeType, EdgeType> me = new Worker<NodeType, EdgeType>(mine, answers[0], algo.getExtender(0));
        // initialize worker for further threads
        for (int i = 1; i < answers.length; ++i) {
            answers[i] = new HashSet<HPListGraph<NodeType, EdgeType>>();
    ...
    public ArrayList<BitSet> getRetMask() {
        ArrayList<BitSet> bulkRet = new ArrayList();
        for (int i = 0; i < bulkRets.length; i ++) {
            bulkRet.addAll(bulkRets[i]);
        }
        return bulkRet;
    }

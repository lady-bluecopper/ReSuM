package eu.unitn.disi.db.resum.matching;

/**
 * ****************************************************************************
 * Compilation: javac Hungarian.java Execution: java Hungarian n Dependencies:
 * FordFulkerson.java FlowNetwork.java FlowEdge.java
 *
 * Solve an n-by-n assignment problem. Bare-bones implementation: - takes n^5
 * time in worst case. - assumes weights are >= 0 (add a large constant if not)
 *
 * For n^4 version: http://pages.cs.wisc.edu/~m/cs787/hungarian.txt Can be
 * improved to n^3
 *
 * TODO: Use BipartiteMatching.java or HopcroftKarp.java to compute max
 * cardinality matching (instead of reducing to maxflow).
 *
 *****************************************************************************
 */
public class HungarianAlgorithm {
    ...
    public HungarianAlgorithm(double[][] weight) {
        n = weight.length;
        xy = new int[n];
        yx = new int[n];
        x = new double[n];
        ...
        for (int i = 0; i < n; i++) {
            System.arraycopy(weight[i], 0, this.weight[i], 0, n);
            xy[i] = -1;
            yx[i] = -1;
        }
        while (true) {
            ...
            for (int i = 0; i < n; i++) {
                if (xy[i] == -1) {
                    ...
                    G.addEdge(new FlowEdge(s, i, 1.0, 1.0));
                }
            }
            for (int j = 0; j < n; j++) {
                if (yx[j] == -1) {
                    G.addEdge(new FlowEdge(n + j, t, 1.0));
                } else {
                    G.addEdge(new FlowEdge(n + j, t, 1.0, 1.0));
                }
            }
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    if (reducedCost(i, j) == 0) {
                        if (xy[i] != j) {
                            G.addEdge(new FlowEdge(i, n + j, 1.0));
                        } else {
                            G.addEdge(new FlowEdge(i, n + j, 1.0, 1.0));
                        }
                    }
                }
            }

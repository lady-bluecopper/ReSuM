...
import sa.edu.kaust.grami.und.decomposer.Decomposer;

import sa.edu.kaust.grami.und.search.SearchLatticeNode;
import eu.unitn.disi.db.resum.und.utilities.DfscodesCache;
import eu.unitn.disi.db.resum.und.utilities.Settings;
import eu.unitn.disi.db.resum.und.utilities.Util;

import sa.edu.kaust.grami.und.csp.ConstraintGraph;
...
public class DFSCode<NodeType, EdgeType> extends
    ...
    private HashIntObjMap<HashSet<Integer>> nonCandidates = null;
     ...
     * @param freqEdgeLabels
     * @param singleGraph
     * @param nonCands
     */
    public DFSCode(ArrayList<Integer> sortedFreqLabels,
            ArrayList<Double> freqEdgeLabels,
            Graph singleGraph,
    ...
    public SearchLatticeNode<NodeType, EdgeType> extend(
        ...
        return new DFSCode<NodeType, EdgeType>(sortedFreqLabels, freqEdgeLabels, singleGraph, Util.clone(nonCandidates))
    ...
    public final Frequency frequency() {
        ...
        if (finalFrequency != null) {
            ...
            if (nonCandidates == null) {
                ...
                DFSSearch df = new DFSSearch(cg, Settings.frequency, nonCandidates, me.getEdgeCount());
                df.searchExistances();

                currentVariables = df.getResultVariables();
                int freq = df.getMNISupport();

                if (Settings.score < 4) {
                    isRelevant = isPatternRelevant(df.getMNIRelSupports(), Settings.frequency);
                    scores = df.getMNIRelSupports();
                } else {
                    isRelevant = isPatternRelevant(df.computeAVGScore(), Settings.frequency);
                    double[] weightSums = df.getAVGScores();
                    if (df.getMNISupport() > 0) {
//                        StringBuilder builder = new StringBuilder().append("{{{");
                        for (int i = 0; i < Settings.actualNumOfEdgeWeights; i++) {
                            scores[i] = weightSums[i] / me.getEdgeCount();
    ...
    public boolean[] isPatternRelevant(double[] a, int threshold) {
        ...
        if (Settings.score < 4) {
            ...
            IntStream.range(0, a.length)
                    .parallel()
                    .forEach(index -> {
                        condIsValid[index] = (a[index] / me.getEdgeCount() >= threshold);
                    });
        }
        return condIsValid;
    }
